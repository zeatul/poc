...
public interface TaskMapper  {
	 ...
	 * @return 查询到的记录
	 */
	TaskDomain load(@Param("id")Integer  id );
	 ...
	 */
	List<TaskDomain> loadDynamic(Map<String,Object> params);
	 ...
	 * @return 符合条件的记录，如果查询不到记录，返回的是空记录数的List
	 */
	List<TaskDomain> loadDynamicPaging(Map<String,Object> params);
	...
	/**
	 * 插入记录
	 * @param TaskDomain 短信发送记录
	 * @return 插入的记录数
	 */
	int insert(TaskDomain taskDomain);
	 ...
	 * @return 更新的记录数
	 */
	int update(TaskDomain taskDomain);
	 ...
	 * @return 更新的记录数
	 */
	int updateWithoutNull(TaskDomain taskDomain);
