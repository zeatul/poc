...
drop index ui_dic_enum on t_dic_enum;

drop table if exists t_dic_enum;
...
(
   ...
   version              unsigned integer comment '版本号',
...
(
   ...
   object_name          varchar(200) comment '名称',
   object_comment       varchar(1024) comment '描述',
   object_order         unsigned integer comment '序号',
   nullable             unsigned integer comment '可否为空(1/0)',
   is_pk                unsigned integer comment '是否为主键(1/0)',
...
/* Table: t_dic_enum                                            */
/*==============================================================*/
create table t_dic_enum
(
   object_id            varchar(50) not null comment '对象ID',
   word_object_id       varchar(50) comment '单词主键',
   enum_key             varchar(50) comment '枚举键',
   enum_value           varchar(50) comment '枚举值',
   version              unsigned integer comment '版本号',
   system_code          varchar(50) comment '系统编码(区分不同项目，不同集团)',
   create_date          timestamp(3) null comment '创建日期',
   update_date          timestamp(3) null comment '更新日期',
...
alter table t_dic_enum comment '枚举';

/*==============================================================*/
/* Index: ui_dic_enum                                           */
/*==============================================================*/
create unique index ui_dic_enum on t_dic_enum
(
   word_object_id,
   enum_key,
   version,
...
(
   ...
   object_name          varchar(200) comment '名称',
   object_comment       varchar(1024) comment '描述',
   is_unique            unsigned integer comment '是唯一索引',
   is_pk                unsigned integer comment '是否为主键(1/0)',
   system_code          varchar(50) comment '系统编码(区分不同项目，不同集团)',
   version              unsigned integer comment '版本号',
   create_date          timestamp(3) null comment '创建日期',
...
(
   ...
   exec_times           unsigned integer comment '已经执行次数',
   max_exec_times       unsigned integer comment '最大允许执行次数',
   ...
   reserved             unsigned integer not null comment '系统保留',
   primary key (object_id)
)
ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(
   object_id            varchar(50) not null comment '对象ID',
   origin_object_id     varchar(50) comment '原单词id',
...
(
   ...
   char_max_length      unsigned integer comment '最大长度',
   char_min_length      unsigned integer comment '最小长度',
   is_only_ascii        unsigned integer comment '是否有超过1个byte长度的的字符',
   max_value            varchar(200) comment '最大值',
   min_value            varchar(200) comment '最小值',
   datetime_precision   unsigned integer comment '时间精度',
   numeric_precision    unsigned integer comment '数据精度',
   numeric_scale        unsigned integer comment '数据小数精度',
   is_enum              unsigned integer comment '是否枚举(yes/no)',
   enum_values          varchar(2000) comment '枚举值',
   system_code          varchar(50) comment '系统编码(区分不同项目，不同集团)',
   version              unsigned integer comment '版本号',
   create_date          timestamp(3) null comment '创建日期',
   update_date          timestamp(3) null comment '更新日期',
