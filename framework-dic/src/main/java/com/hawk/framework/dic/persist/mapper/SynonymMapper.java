...
public interface SynonymMapper  {
	 ...
	 * @return 查询到的记录
	 */
	SynonymDomain load(@Param("objectId")String objectId );
	 ...
	 */
	List<SynonymDomain> loadDynamic(Map<String,Object> params);
	 ...
	 * @return 符合条件的记录，如果查询不到记录，返回的是空记录数的List
	 */
	List<SynonymDomain> loadDynamicPaging(Map<String,Object> params);
	...
	/**
	 * 插入记录
	 * @param SynonymDomain 同义词定义表
	 * @return 插入的记录数
	 */
	int insert(SynonymDomain synonymDomain);
	 ...
	 * @return 更新的记录数
	 */
	int update(SynonymDomain synonymDomain);
	 ...
	 * @return 更新的记录数
	 */
	int updateWithoutNull(SynonymDomain synonymDomain);
