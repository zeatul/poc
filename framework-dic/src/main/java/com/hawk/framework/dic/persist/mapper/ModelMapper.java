...
import com.hawk.framework.dic.persist.domain.ModelDomain;
...
public interface ModelMapper  {
	 ...
	 * @return 查询到的记录
	 */
	ModelDomain load(@Param("objectId")String objectId );
	 ...
	 */
	List<ModelDomain> loadDynamic(Map<String,Object> params);
	 ...
	 * @return 符合条件的记录，如果查询不到记录，返回的是空记录数的List
	 */
	List<ModelDomain> loadDynamicPaging(Map<String,Object> params);
	...
	/**
	 * 插入记录
	 * @param ModelDomain 测试表的元数据使用,包括字段类型,索引,外键,不维护数据
	 * @return 插入的记录数
	 */
	int insert(ModelDomain modelDomain);
	 ...
	 * @return 更新的记录数
	 */
	int update(ModelDomain modelDomain);
	 ...
	 * @return 更新的记录数
	 */
	int updateWithoutNull(ModelDomain modelDomain);
